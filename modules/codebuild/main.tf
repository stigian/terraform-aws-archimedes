...
resource "aws_s3_bucket_acl" "default" {
  count      = local.create_s3_cache_bucket ? 1 : 0
...
resource "aws_s3_bucket_logging" "default" {
  count  = local.create_s3_cache_bucket && var.access_log_bucket_name != "" ? 1 : 0
  bucket = join("", resource.aws_s3_bucket.cache_bucket[*].id)

  target_bucket = var.access_log_bucket_name
  target_prefix = "logs/codebuild/${local.repo_name}/"
}
...
resource "aws_iam_role" "default" {
  name                  = "codebuild-${local.repo_name}"
...
data "aws_iam_policy_document" "role" {
  statement {
    ...
    # Broken in GovCloud, throws CodeBuild is not authorized to perform: sts:AssumeRole...
    # condition {
    #   test     = "StringEquals"
    ...
    #   values = [
    #     "arn:aws:codebuild:${local.aws_region}:${local.aws_account_id}:project/${local.repo_name}"
    #   ]
    # }
...
resource "aws_iam_policy" "default_cache_bucket" {
  ...
  name   = "${local.repo_name}-cache-bucket"
  path   = var.iam_policy_path
...
# TODO:
#   - use var.extra_permissions to add ALB/ECS/EKS permissions per project
#   - limit resources block based on resource type
#   - see terraform-aws-ecs module for starter task_exec permissions
